...
class PrihodiImportingTestCases(unittest.TestCase):
    ...
    vranje_counts_of_parents_and_total = {
        ...
        "ДРУГИ ПОРЕЗИ": 1,
        "ДОНАЦИЈЕ ОД МЕЂ. ОРГАНИЗАЦИЈА": 1,
        "ТРАНСФЕРИ ОД ДРУГИХ НИВОА ВЛАСТИ": 3,
        "ПРИХОДИ ОД ИМОВИНЕ": 11,
        ...
        "МЕМОРАНДУМСКЕ СТАВКЕ ЗА РЕФУНДАЦИЈУ РАСХОДА": 1,
        "ПРИХОДИ ИЗ БУЏЕТА": 1,
        "ПРИМАЊА ОД ПРОДАЈЕ ОСНОВНИХ СРЕДСТАВА": 2,
        "ПРИМАЊА ОД ПРОДАЈЕ ПРИРОДНЕ ИМОВИНЕ": 1,
        "ПРИМАЊА ОД ЗАДУЖИВАЊА": 2,
        "ПРИМАЊА ОД ПРОДАЈЕ ФИН. ИМОВИНЕ": 1
    }
    ...
    def test_counts_for_parent_categories(self):
        ...
        for parent in self.kraljevo_counts_of_parents_and_total:
            self.asserts_for_parent_categories_elements("Kraljevo", parent, self.kraljevo_counts_of_parents_and_total[parent], "prihodi")

        # Test the counts of particular parent category for Novi Beograd municipality
    ...
    def asserts_for_classification_categories_elements(self, municipality, parent_category, expected_value, data_source):
        ...
        result = mongo.datacentar.opstine.find(
            {
                "opstina.latinica": municipality,
                "klasifikacija.broj": parent_category,
                "tipPodataka.slug": data_source
            }
        ).count()
